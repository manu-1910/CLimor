...
class TagsViewModel @Inject constructor(private val tagsUseCase: TagsUseCase, private val sessionManager: SessionManager) : BaseViewModel<TagsViewModel.Input, TagsViewModel.Output>() {
    ...
    var uiTagsRequest = UITagsRequest(
        tag = ""
    )

    var localListTagsSelected: ArrayList<UITags> = ArrayList()
    var localListTags: ArrayList<UITags> = ArrayList()


    private val compositeDispose = CompositeDisposable()

    data class Input(
        val tagsTrigger: Observable<Unit>
    )
